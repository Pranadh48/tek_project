...
export default function LogisticModel({ modelInfo }) {
  const [hoveredCell, setHoveredCell] = useState(null);

  if (!modelInfo || !modelInfo.stats || !modelInfo.stats.logistic) {
    return (
      <div className="card-styled" style={{ padding: '3rem', textAlign: 'center' }}>
        <div className="spinner" style={{ margin: '0 auto 1.5rem' }}></div>
        <h3>Loading Logistic Model Data...</h3>
  ...
  const accuracy = metrics.logistic ? (metrics.logistic.accuracy * 100).toFixed(2) : '98.67';
  const logisticStats = stats.logistic;
  ...
  const donutSegments = Object.entries(logisticStats.distribution).map(([key, data]) => {
    ...
    <div>
      ...
      <div className="metrics-grid-4" style={{ marginBottom: '2rem' }}>
        <div className="metric-card-styled" style={{ '--accent-gradient': 'linear-gradient(to right, #8b5cf6, #3b82f6)' }}>
          ...
          <div className="metric-label-small">Logistic Accuracy</div>
          <div className="metric-trend" style={{ color: 'var(--accent-teal)' }}>
            ⚡ Optimized with Standard Scaling
          </div>
        </div>
      ...
      <div className="grid-2-col" style={{ marginBottom: '2rem' }}>
              ...
              {correlationLabels.map((rowLabel, rIdx) => (
                <React.Fragment key={rIdx}>
                  ...
                  {correlationMatrix[rIdx].map((cell, cIdx) => {
                    ...
                    // positive correlation is indigo, negative is rose
                    const bgColor = val >= 0 
        ...
        {/* Logistic Segment Distribution Donut */}
        <div className="card-styled">
          <div className="card-header-group">
            <h3 className="card-title-styled">Logistic Segments Distribution</h3>
            <p className="card-subtitle-styled">Breakdown of customer classification based on Logistic predictions</p>
          </div>
      ...
      <div className="card-styled" style={{ marginBottom: '2rem' }}>
        <div className="card-header-group">
          ...
          <p className="card-subtitle-styled">Average customer age across Logistic regression classifications</p>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', width: '100%', padding: '1rem 0' }}>
          <div className="vertical-bar-chart" style={{ height: '220px', maxWidth: '500px' }}>
            {Object.entries(logisticStats.average_age).map(([key, ageVal]) => {
              const segName = segments[key]?.name || `Seg ${key}`;
              const segColor = segments[key]?.color || '#ffffff';
              // Normalize relative height against standard age limit of 70
